...
if(!IsSet($update))	{
	...
	elseif($uhome < 0 OR $ubarrack < 0 OR $ufarm < 0 OR $uwp < 0 OR $ugm < 0 OR $uim < 0 OR $ulmill < 0)	{
		echo"<div align=center><font class=yellow>You cannot demolish negative or 0 buildings.</font></div>";
		include("include/S_DBUILD.php");
		die();
	}
	elseif ($uhome > $home OR $ufarm > $farm OR $uwp > $wp OR $ubarrack > $barrack OR $ulmill > $lmill)	{
