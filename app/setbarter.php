...
echo "<center> <b class=reg> | <a href=barter.php> -Game- </a> | <a href=setbarter.php> -Settlement- </a> | <a href=guildbarter.php> -Guild- </a> | </b></center><br>";	

if(!IsSet($barter))	{
	 include("include/set_barter.php");
}
else	{
...
//	select ____ from barter table	
	...
	if($b_type == "Rottweiler")	 {
		$newdogs = $dogs + $b_amount;
		mysql_query("UPDATE military SET dogs='$newdogs' WHERE email='$email' AND pw='$pw'");
...
//	various invalid checks
	elseif($type != "Scientist" AND $type != "Wizard" AND $type != "Priest" AND $type != "Warrior" AND $type != "Thief" AND $type != "Recruit" AND $type != "Land" AND $type != "Mountain" AND $type != "Rottweiler")	 {	echo"<div align=center><font class=yellow>Invalid type!</font></div><br><br>";	die();	 }
...
//	number of unit checks
	elseif($type == "Rottweiler" AND $dogs < $amount)	{	echo"<font class=yellow><div align=center>You don't have that much many rottweilers!</font></div><br><br>";	die();	 }
...
//	invalid number checks
	elseif($type == "Rottweiler" AND $dogs <= 0)	{	echo"<font class=yellow><div align=center>Invaild number.</font></div><br><br>";	die();	 }
	...
	mysql_query("INSERT INTO barter (seller, cost, type, amount, barterid, userid, method, page, setid)	VALUES	('$ename', '$cost', '$type', '$amount', '$maxbid', '$userid', '$method', '$page', '$setid') ");
...
if(!IsSet($end))	{
	...
	if($b_userid != $userid)	{
		echo"<font class=yellow><div align=center>You cannot cancel other empire barters.</font></div><br><br>";
		die();
	}
	if($b_type == "Rottweiler")	{
		$newdogs = $dogs + $b_amount;
