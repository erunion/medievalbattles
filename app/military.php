...
else
{
			...
			$totalunits =  $warriors + $wizards + $priests + $archers;
	...
	elseif($gp < ($uthief * 200) + ($uexplorer * 1000) + ($uscientist * 500) + ($priestc * $upriest) + ($wizardc * $uwizard) + ($warriorc * $uwarrior) + ($archerc * $uarcher)) 
	...
	elseif($totalunits >= $barrack_cap)
	...
	else
		{
	 		...
	 		$gp = $gp - ((($uthief * 200) + ($uscientist * 500) + ($uexplorer * 1000)) + (($priestc * $upriest) + ($wizardc * $uwizard) + ($warriorc * $uwarrior) + ($archerc * $uarcher)));
	 		$gp = round($gp);
