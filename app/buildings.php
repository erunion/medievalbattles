...
<table class=outer border="0" cellpadding="1" cellspacing="0"  width="100%">
 <TR>
  ...
  <br><br>
<center><b class=reg>| <a href="dbuildings.php"> -Demolish- </a> | </b></center>
	 ...
	 <td><br>
	  ...
	  <table border="1" bordercolor="#000000" align=center width="80%">
	   <tr>
		...
		<tr>
	    <td colspan=7 class=main2>It costs <?php echo"$bcost"; ?> gp to construct a building</td>
	   ...
	   <tr>
		...
		<td class=inner2><?php echo"$aland"; ?></td>
...
<br><br>
	   ...
	   <tr>
	    <td colspan=7 class=main2>It costs <?php  echo"$bmcost"; ?> gp to construct a mine</td>
	   ...
	   <tr>
		...
		<td class=inner2><?php echo"$amts"; ?></td>
...
{
	if($amts < $ugm + $uim) 
	{print "You do not have enough available mountains to build on.";die();
	}
	elseif($aland < $uhome + $ubarrack + $ufarm + $ulab)
		{print "You do not have enough available land to build on.";die();
		}
	elseif ($gp < (20 * ($ugm + $uim)) * (($gm + $im) * .2) + (20 * ($uhome + $ubarrack + $ufarm + $ulab)) * (($home + $barrack + $farm + $lab) * .2)) 
	...
	$gp = $gp - ((20 * ($ugm + $uim)) * (($gm + $im) * .2) + (20 * ($uhome + $ubarrack + $ufarm + $ulab)) * (($home + $barrack + $farm + $lab) * .2));
	 $gp=floor($gp);
	...
	$forexp2 = $exp2 + (($uhome + $ubarrack + $ufarm + $ulab) * $landexp) + (($ugm + $uim) * $mtexp); 
	 ...
	 @mysql_connect (localhost, ziccarelli, pa724);
	  ...
	  mysql_query("UPDATE buildings SET dgm =dgm+$ugm WHERE email='$email' AND pw='$pw'");  
	  mysql_query("UPDATE buildings SET dim=dim+$uim WHERE email='$email' AND pw='$pw'");
	  mysql_query("UPDATE buildings SET amts =\"$amts\" WHERE email='$email' AND pw='$pw'");
	  mysql_query("UPDATE buildings SET dhome=dhome+$uhome WHERE email='$email' AND pw='$pw'");  
	  mysql_query("UPDATE buildings SET dbarrack =dbarrack+$ubarrack WHERE email='$email' AND pw='$pw'");  
	  mysql_query("UPDATE buildings SET dfarm = dfarm+$ufarm WHERE email='$email' AND pw='$pw'"); 
	  mysql_query("UPDATE buildings SET dlab = dlab+$ulab WHERE email='$email' AND pw='$pw'");
