...
<form type=post action=suicivatk.php>
...
if(!IsSet($attack))	{
	include("include/attack/scdrop.php");
	include("include/attack/sctable.php"); 
}
else	{  	
	...
	elseif($usuicide > 0 && $res[r14pts] < 125000)	{
		echo"<div align=center><font class=yellow>You have to research Pyrotechnics.<br><br></font></div>";
		include("include/attack/scdrop.php");
		include("include/attack/sctable.php"); 
		die();
	}
	...
	elseif($cdefense <= $evpower)	{
		...
		$sid = $msid + 1;
		mysql_query("INSERT INTO setnews (date, news, setid, sid)	VALUES	('$clock', '<font class=yellow>$ename ($setid) failed to suicide attack $evu[ename] ($evu[setid])</font>', '$setid', '$sid') ");
					
		if($gid[0] != "")	{
			$gnid = $mgnid + 1;
			mysql_query("INSERT INTO guildnews (date, news, gid, gnid)	VALUES	('$clock', '<font class=yellow>$ename ($setid) failed to suicide attack $evu[ename] ($evu[setid])</font>', '$gid[0]', '$gnid') ");
		}
		
		$sid = $msid + 1;
		mysql_query("INSERT INTO setnews (date, news, setid, sid)	VALUES	('$clock', '<font class=orange>$evu[ename] ($evu[setid]) successfully defended their empire against $ename ($setid)</font>', '$evu[setid]', '$sid') ");
					
		if($tgid[0] != "")	{
			$gnid = $mgnid + 2;
			mysql_query("INSERT INTO guildnews (date, news, gid, gnid)	VALUES	('$clock', '<font class=orange>$evu[ename] ($evu[setid]) successfully defended their empire against $ename ($setid)</font>', '$tgid[0]', '$gnid') ");
		}
		...
		echo"<div align=center><font class=yellow>We have failed to attack $evu[ename]($evu[setid])!</font><br><br>
			<font class=orange>$your_losses<br><br></font></div>";
